...
- База данных: `PostgreSQL` [ДОПУЩЕНИЕ: исходя из `asyncpg` и Postgres UUID-типа]
- Миграции: `Alembic`
- Валидация: `Pydantic v2`
- Object storage: `MinIO` / S3-compatible storage через `boto3`
...
Наблюдения:
- поддерживаются как минимум два auth provider: `email` и `telegram`;
- email/password и Telegram auth объединяются в одну модель пользователя;
...
Особенности:
- чат сейчас user-centric, а не pet-centric;
- история сообщений сохраняется в БД;
- генерация ответа выполняется синхронно в рамках API запроса.
...
Наблюдение:
- версионирование API (`/api/v1`) пока не используется;
- на текущем этапе решено сохранить маршруты без префикса версии;
...
- repository abstraction отсутствует, сервисы работают с `AsyncSession` напрямую;
...
- shared user должен иметь доступ к документам, карте, рекомендациям и ИИ-чату питомца.

Это частично расходится с текущей backend-реализацией и потребует корректировки permission model, особенно для LLM-чата.
...
- нет retry policy;
- нет rate limiting;
- нет привязки контекста ответа к данным конкретного питомца;
- ошибки внешнего API пробрасываются как `HTTPException(502)`.

Зафиксированное продуктовое направление:
- ИИ-чат должен быть привязан к питомцу;
- данные профиля питомца должны подключаться как контекст только по явному выбору пользователя;
- доступ ИИ к документам питомца пока не является обязательным сценарием и отложен на будущую итерацию.
...
Текущий API карты:
- не фильтрует точки по пользователю или питомцу;
...
- Нет выделенного repository layer.
...
- Access token передается через bearer header, а refresh flow работает по телу запроса, а не через `httpOnly cookie`.
...
- Не видно audit logging по security-событиям.

### 17.3 Storage / files
- Нет явных лимитов по типу и размеру файла на уровне API-слоя.
- Нет вирус-сканирования файлов. [ДОПУЩЕНИЕ: по коду не наблюдается]
- Нет фоновой очистки orphaned objects, если клиент не вызвал `complete`.

### 17.4 LLM
- Нет очередей/фоновых задач.
- Нет timeout fallback или circuit breaker поверх внешней LLM-интеграции.
- Нет ограничения длины истории чата.
...
- Не видно health endpoints, readiness/liveness probes.
...
### Высокий приоритет
- Зафиксировать permission matrix в отдельном документе.
- Защитить `upload-data` маршруты административной авторизацией.
- Формализовать file limits и content-type policy.
- Уточнить production-safe формат ошибок для `500`.
- Подготовить backend к pet-centric модели ИИ-чата и расширенным правам `shared user`.
...
### Дальнейшее развитие
- Привязать LLM-контекст к данным конкретного питомца.
