...
class PetsRepository:
    ...
    async def get_pet_details(self, pet_id: int, user_id: str | int) -> Optional[Dict[str, Any]]:
        result = await self.db.execute(
            ...
            {"pet_id": pet_id, "user_id": self._normalize_user_id(user_id)},
        )
        row = result.mappings().first()
        return dict(row) if row else None

    async def get_pet_short_info(self, pet_id: int, user_id: str | int) -> Optional[Dict[str, Any]]:
        result = await self.db.execute(
            text(
                """
                SELECT
    ...
    async def pet_exists_for_user(self, pet_id: int, user_id: str | int) -> bool:
        result = await self.db.execute(
