...
class DocumentsRepository:
    ...
    async def get_pet_documents(self, pet_id: int, user_id: str | int) -> List[Dict[str, Any]]:
        result = await self.db.execute(
            text(
                """
                SELECT d.custom_name, dt.document_name
                FROM pet_documents d
                JOIN pets_info p ON p.id = d.pet_id
                LEFT JOIN documents_types dt ON dt.id = d.document_id
