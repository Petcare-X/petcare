...
Requires the docker-compose stack to be running:
    cd apps/backend/src/mcp-server && make up
...
TEST_DOCUMENT_OBJECT_KEY = "pets/integration/test-doc.txt"
...
async def _seed_db() -> dict:
    ...
    try:
        ...
        doc_id = await conn.fetchval(
            ...
            TEST_DOCUMENT_OBJECT_KEY,
            "text/plain",
            len(TEST_DOCUMENT_CONTENT),
            doc_type_id,
        )
        ...
        clinic_id = await conn.fetchval(
            ...
            False,
            "+7-999-000-00-01",
            "https://integration-vet.example.com",
        )
...
async def client(auth_token) -> AsyncGenerator[httpx.AsyncClient, None]:
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, headers=headers, timeout=10) as http_client:
        yield http_client
